...
In SQL, DQL (Data-Query Language) is a subset of commands that is focused on retrieving data from tables. The knowledge on this specific subset of SQL is important so the actor can retrieve the data in a way it will help us answer questions by an analytical approach. 
...
# print(sqlfluff.fix(query)[1:])

print(f"```sql\n{sqlfluff.fix(query)[1:]}\n```")
```